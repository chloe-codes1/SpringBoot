...
public class smPageMaker {
	...
	private int pageBlockperCount = 5;
	...
	public void prevnext(int pagenum) {
		...
		if(pagenum>0 && pagenum<= pageBlockperCount) {//첫번째 페이지블록 관련
			prev=false;
			next=true;
			
		}//이전 페이지 블록으로 가는 화살표는 보이지 않게 된고 다음페이지로 가는 화살표는 보이게 된다.
		else if(lastblock == currentblock)//마지막 페이지 블록관련
	...
	public void setStartPage(int currentblock) { //페이지 블록을 구하는 함수
		...
		this.startPage = (currentblock*pageBlockperCount)-4;
	}
	...
	public void setCurrentblock(int pagenum) {
		...
		this.currentblock = pagenum/pageBlockperCount; //여기서는 결과값이 0이다 인트형이기때문이다 0페이지가 없으므로 if문으로
		if(pagenum%pageBlockperCount>0) {//나머지가 0.2이므로 0보다 크기때문에 1을 더해준다 즉 첫번째 페이지 블록이 된다.
			this.currentblock++;
		}
	}
	...
	public void setLastblock(int totalcount) {
		...
		this.lastblock = totalcount/(pageBlockperCount*this.contentnum);
		if(totalcount %(pageBlockperCount*this.contentnum)>0) {
			this.lastblock++;
		}
